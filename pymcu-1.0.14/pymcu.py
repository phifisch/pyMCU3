...
def mcuScan(baudrate=115200):
    ...
    if os.name == 'nt':
        for i in range(256):
            try:
                s = serial.Serial('COM'+str(i),baudrate,timeout=1)
                print('got something at COM'+str(i))
                s.write( ('i' + chr(1) + chr(0)).encode())
                checkID = s.read(6)
                if b'pymcu' in checkID:
        ...
        for i in os.listdir('/dev/'):
            if 'ttyUSB' in i or 'usbserial' in i:
                try:
                    ...
                    pass
    # make sure to close Serial or it remains blocked
    try:
        s.close()
    except NameError: #isn't defined
        pass
    
    return available

def guess_pyMCU_port(self):
    """a new way of doing things"""
    po = serial.tools.list_ports.comports()
    pymcuPort = tuple(filter(lambda p: 'FTDI' in p.manufacturer , po))
    if len(pymcuPort) == 0:
        raise NotConnectedError('no Arduino detected')
    return pymcuPort[0].device
...
class mcuModule:
    def __init__(self, port=None, baudrate=115200):
        ...
        if port == None:
            ...
            if len(findmcu) > 0:
                self.mcuserial = serial.Serial(list(findmcu.items())[0][0],baudrate, timeout=1)
                self.mcuserial.write( ('i' + chr(1) + chr(0)).encode() )
                ...
                for baud in self.baudList:
                    ...
                    if len(findmcu) > 0:
                        self.mcuserial = serial.Serial(list(findmcu.items())[0][0],baud, timeout=1)
                        self.mcuserial.write( ('i' + chr(1) + chr(0)).encode() )
                        checkID = self.mcuserial.read(6)
                        self.mcuType = int(checkID[5:])
                        self.active = self.mcuserial.isOpen()
    ...
    def mcuVersion(self):
        ...
        Usage: mcuVersion()"""
        self.mcuserial.write( ('v' + chr(0) + chr(0)).encode() )
        return self.mcuserial.read(30).decode()
    ...
    def mcuSetBaudRate(self, baudrate):
        ...
        if baudrate >= 0 and baudrate <= 12:
            self.mcuserial.write( ('b' + chr(baudrate) + chr(0)).encode() )
            eb = b""
            while eb != b"eb":
    ...
    def digitalState(self,pinNum, io):
        ...
        if type(io) == str:
            if io.lower() == 'input':
                self.mcuserial.write( ('=' + chr(pinNum) + chr(1)).encode() )
            elif io.lower() == 'output':
                self.mcuserial.write( ('=' + chr(pinNum) + chr(0)).encode() )
            else:
    ...
    def digitalRead(self,pinNum):
        ...
        Returns 0 or 1"""
        self.mcuserial.write( ('r' + chr(pinNum) + chr(0)).encode() )
    ...
    def spiEnable(self, clockPolarity, clockFreq, clockSelect, clockSample):
        ...
        if self.mcuType == 0:
            ...
            self.mcuserial.write( ('c' + chr(sspCon1) + chr(cFreq)).encode() )
            self.mcuserial.write(chr(sspStat).encode())
        else:
    ...
    def spiDisable(self):
        ...
        if self.mcuType == 0:
            self.mcuserial.write( ('h' + chr(0) + chr(0)).encode() )
        else:
            sys.stderr.write('Your current pyMCU board does not support this feature.\n')
    ...
    def spiTransfer(self, writeData, delay = 200):
        ...
        if self.mcuType == 0:
            ...
            bL = delay & 255
            bH = delay >> 8
            self.mcuserial.write(('n' + chr(bH) + chr(numBytes)).encode() )
            self.mcuserial.write( (chr(bL) + byteArray).encode() )
            spir = self.mcuserial.read(numBytes)
            spidata = [ord(i) for i in spir]
            # for i in spir:
    ...
    def pwmOn(self, pwmPin):
        ...
        self.mcuserial.write( ('m' + chr(pwmPin) + chr(1)).encode() )
    ...
    def pwmOff(self, pwmPin):
        ...
        self.mcuserial.write( ('m' + chr(pwmPin) + chr(0)).encode() )
    ...
    def pwmPeriod(self, preScaler, preScalerOffset):
        ...
        self.mcuserial.write( ('P' + chr(t4con) + chr(preScalerOffset)).encode() )
    ...
    def pwmDuty(self, pwmPin, duty = 500):
        ...
        if duty < self.PWMPins[self.mcuType]['MinDuty'] or duty > self.PWMPins[self.mcuType]['MaxDuty']:
            ...
            return
        self.mcuserial.write( ('w' + chr(pwmPin) + chr(0)).encode() )
        bL = duty & 255
        bH = duty >> 8
        self.mcuserial.write( ('w' + chr(bL) + chr(bH)).encode() )
    ...
    def analogRead(self,pinNum):
        ...
        self.mcuserial.write( ('a' + chr(pinNum) + chr(0)).encode() )
    ...
    def soundOut(self, pinNum, note = 100, duration = 10):
        ...
        if self.mcuType == 0:
            ...
            if duration < 0 or duration > 255:
                ...
                return
            self.mcuserial.write( ('s' + chr(pinNum) + chr(note)).encode())
            self.mcuserial.write( ('s' + chr(duration) + chr(0)).encode() )
            sd = b""
            while sd != b"sd":
    ...
    def dtmfOut(self, pinNum, dtmfNum):
        ...
        if self.mcuType == 0:
            if dtmfNum < 0 or dtmfNum > 9:
                ...
                return
            self.mcuserial.write( ('d' + chr(pinNum) + chr(dtmfNum)).encode() )
            dd = b""
            while dd != b"dd":
    ...
    def freqOut(self, pinNum, duration = 1000, freq1 = 0, freq2 = 0 ):
        ...
        if self.mcuType == 0:
            ...
            if freq2 < 0 or freq2 > 65535:
                ...
                return
            if freq2 != 0:
                self.mcuserial.write( ('f' + chr(pinNum) + chr(2)).encode() )
            else:
                self.mcuserial.write( ('f' + chr(pinNum) + chr(1)).encode() )
            bL = freq1 & 255
            bH = freq1 >> 8
            self.mcuserial.write( ('f' + chr(bL) + chr(bH)).encode() )
            if freq2 != 0:
                bL = freq2 & 255
                bH = freq2 >> 8
                self.mcuserial.write( ('f' + chr(bL) + chr(bH)).encode() )
    ...
    def i2cRead(self, control, *args ):
        ...
        if self.mcuType == 0:
            if len(args) == 2:
                numBytes = args[1]
                address = args[0]
                self.mcuserial.write( ('k' + chr(4) + chr(numBytes)).encode() )
                self.mcuserial.write( (chr(1) + chr(control) + chr(address)).encode() )
            elif len(args) == 1:
                numBytes = args[0]
                self.mcuserial.write( ('k' + chr(4) + chr(numBytes)).encode() )
                self.mcuserial.write( (chr(0) + chr(control) + chr(0)).encode() )
            else:
    ...
    def i2cWrite(self, control, *args):
        ...
        if self.mcuType == 0:
            ...
            if numBytes > 64:
                sys.stderr.write('Number of bytes sent to long: [64 Bytes Max]\n')
                return
            self.mcuserial.write( ('j' + chr(4) + chr(numBytes)).encode() )
            if len(args) == 1:
                self.mcuserial.write( (chr(0) + chr(control) + chr(0) + byteArray).encode() )
            else:
                self.mcuserial.write( (chr(1) + chr(control) + chr(address) + byteArray).encode() )
            ic = b""
            while ic != b"ic":
    ...
    def owWrite(self, pinNum, mode, writeData):
        ...
        if self.mcuType == 0:
            ...
            numBytes = len(byteArray)
            self.mcuserial.write( ('o' + chr(pinNum) + chr(numBytes)).encode() )
            self.mcuserial.write( ('O' + chr(mode) + byteArray).encode() )
            iw = b""
            while iw != b"iw":
    ...
    def owRead(self, pinNum, mode, numBytes ):
        ...
        returns a list item of read data"""
        if self.mcuType == 0:
            self.mcuserial.write( ('t' + chr(pinNum) + chr(numBytes)).encode() )
            self.mcuserial.write(chr(mode).encode())
            iw = self.mcuserial.read(numBytes)
            oneWire = [ord(i) for i in iw]
            # for i in iw:
    ...
    def irDecode(self, pinNum):
        ...
        """
        if self.mcuType == 0:
            self.mcuserial.write( ('z' + chr(pinNum) + chr(0)).encode() )
    ...
    def pulseCount(self, pinNum, duration):
        ...
        if self.mcuType == 0:
            if duration > 0 and duration < 60000:
                bL = duration & 255
                bH = duration >> 8
                self.mcuserial.write( ('C' + chr(pinNum) + chr(0)).encode() )
                self.mcuserial.write( (chr(0) + chr(bL) + chr(bH)).encode() )
    ...
    def pulseIn(self, pinNum, state):
        ...
        if self.mcuType == 0:
            self.mcuserial.write( ('u' + chr(pinNum) + chr(state)).encode() )
            pl = self.mcuserial.read(4)
            pl = int(b'0x' + pl, 0)
            return int(pl)
        else:
    ...
    def pulseOut(self, pinNum, pulse, count):
        ...
        if self.mcuType == 0:
            ...
            bL = pulse & 255
            bH = pulse >> 8
            self.mcuserial.write( ('p' + chr(pinNum) + chr(count)).encode() )
            #time.sleep(0.1)
            self.mcuserial.write( ('p' + chr(bL) + chr(bH)).encode() )
            pd = b""
            while pd != b"pd":
    ...
    def pinToggle(self,pinNum):
        ...
        if type(pinNum) == int:
            self.mcuserial.write( ('g' + chr(pinNum) + chr(0)).encode() )
        if type(pinNum) == list or type(pinNum) == tuple:
            bL,bH = self.__bitmask__(pinNum)
            self.mcuserial.write( ('g' + chr(bL) + chr(bH)).encode() )
    ...
    def pinHigh(self,pinNum):
        ...
        if type(pinNum) == list or type(pinNum) == tuple:
            bL,bH = self.__bitmask__(pinNum)
            self.mcuserial.write( ('+' + chr(bL) + chr(bH)).encode() )
    ...
    def eepromWrite(self, address, value ):
        ...
        if type(value) == int:
            if value < 0 or value > 255:
                ...
                return
            self.mcuserial.write( ('e' + chr(address) + chr(value)).encode() )
            ed = b""
            while ed != b"ed":
        ...
        if type(value) == str:
            if ord(value) < 0 or ord(value) > 255:
                sys.stderr.write('EEPROM Value is Out of Range: [0-255]\n')
                return
            self.mcuserial.write( ('e' + chr(address) + chr(ord(value))).encode() )
            ed = b""
            while ed != b"ed":
                ed = self.mcuserial.read(2)
    ...
    def eepromRead(self, address ):
        ...
        if address < 1 or address > 255:
            sys.stderr.write('EEPROM Address is Out of Range: [0-255]\n')
            return
        self.mcuserial.write( ('x' + chr(address) + chr(0)).encode() )
        return self.mcuserial.read(1).decode()
    ...
    def serialRead(self, pinNum, mode, timeout, numBytes ):
        ...
        if numBytes > 64:
            ...
            return
        self.mcuserial.write( ('y' + chr(pinNum) + chr(numBytes)).encode() )
        tmL = timeout & 255
        tmH = timeout >> 8
        self.mcuserial.write( ('y' + chr(mode) + chr(tmL) + chr(tmH)).encode() )
        self.mcuserial.timeout = int(timeout / 1000)
        sr = self.mcuserial.read(numBytes).decode()
        self.mcuserial.timeout = 1
        return sr
    ...
    def serialWrite(self, pinNum, mode, serialData ):
        ...
        if type(serialData) == str:
            if len(serialData) > 64:
                ...
                return
            self.mcuserial.write( ('q' + chr(pinNum) + chr(len(serialData))).encode() )
            self.mcuserial.write( ('q' + chr(mode) + serialData).encode() )
        if type(serialData) == int:
            if serialData >= 0 and serialData <= 255:
                self.mcuserial.write( ('q' + chr(pinNum) + chr(1)).encode() )
                self.mcuserial.write( ('q' + chr(mode) + chr(serialData)).encode() )
            else:
        ...
        if type(serialData) == list or type(serialData) == tuple:
            for sd in serialData:
                if type(sd) == str:
                    if len(sd) <= 64:
                        self.mcuserial.write( ('q' + chr(pinNum) + chr(len(sd))).encode() )
                        self.mcuserial.write( ('q' + chr(mode) + sd).encode() )
                        time.sleep(0.1)
                if type(sd) == int:
                    if sd >= 0 and sd <= 255:
                        self.mcuserial.write( ('q' + chr(pinNum) + chr(1)).encode() )
                        self.mcuserial.write( ('q' + chr(mode) + chr(sd)).encode() )
                        time.sleep(0.1)
    ...
    def lcd(self,line=0,text=''):
        ...
        if len(text) > 32:
            ...
            return
        if line == 0:
            self.mcuserial.write( ('l' + chr(0) + chr(0)).encode() )
            time.sleep(self.lcdDelay * 5)
        else:
            self.mcuserial.write( ('l' + chr(line) + chr(len(text))).encode() )
            self.mcuserial.write(text.encode())
    ...
    def reset(self):
        ...
        Example: reset()
        """
        self.mcuserial.write( ('!' + chr(0) + chr(0) + chr(0) + chr(0)).encode() )
